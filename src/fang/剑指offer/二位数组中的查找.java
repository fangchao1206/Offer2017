package fang.剑指offer;

/**
 * 在一个二维数组中，每一行都按照从左到右递增的
 * 顺序排序，每一列都按照从上到下递增的顺序排序。
 * 请完成一个函数，输入这样的一个二维数组和一个整数，判断数组中是否含有该整数。
 * Created by fangchao05 on 2017/7/31.
 * <p>
 * <p>
 * 规律：首先选取数组中右上角的数字。如果该数字等于要查找的数字，查找过程结束：
 * 如果该数字大于要查找的数字，剔除这个数字所在的列：如果该数字小于要查找的数字，剔除这个数字所在的行。
 * 也就是说如果要查找的数字不在数组的右上角，则每－次都在数组的查找范围中剔除）行或者一列，这样每一步都可以缩小
 * 查找的范围，直到找到要查找的数字，或者查找范围为空。
 */
public class 二位数组中的查找 {
  static   public boolean Find(int target, int[][] array) {
        if (array == null || array[0].length < 1 || array.length < 1) {
            return false;
        }
        int r = array.length;//总行数
        int c = array[0].length;//总列数
        int row = 0; // 起始开始的行号
        int col = c - 1; // 起始开始的列号
        while (row >= 0 && row < r && col >= 0 && col < c) {
            if (array[row][col] == target) {
                return true;
            } else if (array[row][col] > target) {
                col--;
            }else{
                row++;
            }
        }
        return false;
    }

    public static void main(String[] args) {
        int ass[][] = new int[][] {{1, 2,4,6,7}, {3, 43, 53,88,100}};
        System.out.println(Find(8,ass));
    }

}
